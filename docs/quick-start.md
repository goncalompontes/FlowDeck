...
`/resume` reads `STATE.md` and the active `PLAN.md` and restores full context for all agents.

---

## Tips

> **Check status at any time** — `/progress` prints the current state, active plan, and a summary of recent results without modifying anything.

> **Context after a restart** — always run `/resume` at the start of a new OpenCode session on a project that was previously active. Agents have no memory between sessions without it.
...
> **Skip steps for small tasks** — for a quick bug fix, you do not need to run `/discuss` and `/plan`. Use `/fix-bug` directly and let `@debug-specialist` handle the full cycle.
