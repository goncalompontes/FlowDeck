...
const CONTRACTS: AgentContract[] = [
  ...
  },
  {
    ...
    allowedTools: ["read", "write", "edit", "glob", "grep", "capture-lesson", "review-lessons"],
  ...
  },
  {
    ...
    role: "Default execution worker for simple, direct tasks. Handles quick-answer, inspect-only, simple-edit, and direct-stock-tools workflows.",
    allowedTaskTypes: ["execution", "quick-answer", "inspect-only", "simple-edit", "direct-stock-tools"],
    requiredInputs: ["routed task from orchestrator"],
    expectedOutputFields: ["files_touched", "summary", "verification"],
    allowedTools: ["read", "write", "edit", "bash", "glob", "grep", "capture-lesson", "review-lessons"],
    forbiddenActions: [
      "act as orchestrator",
      "route work to other agents",
      "silently expand scope beyond routed task",
    ],
    escalationConditions: [
      "task touches more than 5 files",
      "task requires architectural decisions",
      "task involves security-sensitive paths",
    ],
    stopConditions: ["task complete", "verification passed", "report submitted to orchestrator"],
    successCriteria: [
      "task executed exactly as routed",
      "completion report includes files touched, summary, and verification",
      "scope not expanded beyond routed task",
    ],
  },
  {
    agent: "debug-specialist",
    role: "Diagnose bugs through systematic root cause analysis. Read-only investigation.",
    allowedTaskTypes: ["debug", "bug-investigation", "root-cause-analysis"],
    requiredInputs: ["bug report or failing behavior", "stack trace or reproduction steps"],
    expectedOutputFields: ["root_cause", "explanation", "recommended_fix"],
    allowedTools: ["read", "glob", "grep", "capture-lesson", "review-lessons"],
    forbiddenActions: [
      "modify source files",
      "run shell commands that change state",
      "fix the bug — only diagnose",
    ],
    escalationConditions: [
      "reproduction steps missing",
      "root cause outside the listed files",
      "build or test infrastructure broken",
    ],
    stopConditions: ["root cause identified", "report submitted to orchestrator"],
    successCriteria: [
      "root cause traced to a specific call site or invariant violation",
      "hypothesis is reproducible",
      "report handed back to orchestrator for routing to a fixer",
    ],
  },
  {
    agent: "build-error-resolver",
    role: "Fix build errors, compilation failures, and dependency issues. Read/write.",
    allowedTaskTypes: ["build-fix", "type-fix", "dependency-fix", "tsc-fix"],
    requiredInputs: ["build error output", "affected files"],
    expectedOutputFields: ["files_modified", "fix_summary", "verification"],
    allowedTools: ["read", "write", "edit", "bash", "glob", "grep", "capture-lesson", "review-lessons"],
    forbiddenActions: [
      "change unrelated source files",
      "bump dependencies to mask the error",
      "weaken type definitions to silence tsc",
    ],
    escalationConditions: [
      "fix requires architectural change",
      "dependency has a known CVE",
      "fix touches sensitive paths (auth, crypto, db)",
    ],
    stopConditions: ["build passes", "types check", "no regressions in existing tests"],
    successCriteria: [
      "build error eliminated at the root cause",
